...
void *mm_realloc(void *ptr, size_t size)
{
    ...
    // 요청한 payload 크기를 실제 block 크기로 변환
    if (size <= DSIZE) {
        asize = 2 * DSIZE;
    }
    else {
        // 사용자가 요청한 payload 크기에 header/footer 크기 더한 뒤, 그 값을 8바이트 배수로 올림해서 실제 block 크기 asize 만드는 식
        asize = DSIZE * ((size + DSIZE + (DSIZE - 1)) / DSIZE);
    }
    ...
    // 경우 1 : 현재 block만으로 충분한 경우
    if (oldsize >= asize)
    {
        // 남는 공간이 최소 free block 크기 이상이면 뒤쪽 남는 공간은 새 free block으로 split
        if ((oldsize - asize) >= MINBLOCKSIZE)
        {
            // 현재 block의 크기를 새 요청 크기로 줄이고 allocated 상태로 다시 기록
            PUT(HDRP(ptr), PACK(asize, 1));
            PUT(FTRP(ptr), PACK(asize, 1));

            // 현재 block 뒤쪽에 남은 공간을 새 free block으로 만들고, 필요하면 coalesce
    ...
    // 경우 2 : 바로 뒤 free block과 합쳐서 제자리 확장이 가능한 경우

    // 현재 block 바로 뒤 block의 주소 구하기
    next_bp = NEXT_BLKP(ptr);
    if (!GET_ALLOC(HDRP(next_bp)))
    {
        // 다음 block 크기를 읽고,
        nextsize = GET_SIZE(HDRP(next_bp));
        // 현재 block과 다음 block 합쳤을 때 전체 크기 계산
        ...
        if (total >= asize)
        {
            ...
            // 합친 뒤에도 남는 공간이 충분히 크다면, 필요한 만큼한 할당하고 나머지는 free block 으로 split
    ...
    // 경우 3 : 제자리에서 해결할 수 없어 새 block을 할당하는 경우
    ...
    // 새 block에 들아갈 수 있는 만큼만 기존 payload 복사
